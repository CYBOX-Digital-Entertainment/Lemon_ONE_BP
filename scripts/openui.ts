...
//주면에 플래이어가 있으면 ui띄움
export function openui(entity: Entity[]) {
    system.run(() => {
        let ui = new ActionFormData().body(``).button(`운전자 탑승`).button(`트렁크 열기`).button(`트렁크 닫기`)
        entity.forEach(entity => {
            if (entity.getTags().find(f => f.startsWith("id:"))?.slice(3) === undefined) return;
            if (entity.hasTag(`탑승`)) return
...
function ride(player: Player, ent : Entity) {
